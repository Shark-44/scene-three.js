...
// a) creation d'une class scene

class Scene {
   // b) methode pour creer un instance avec les proprietés 
    constructor (image, points) {
        this.image = image
        this.points = points
        this.sprite = [] // tableau de l'ensemble des sprites
    }

    // c) methode pour creer une scene
    createScene (scene) {
        this.scene = scene
        const geometry = new THREE.SphereGeometry( 50, 32, 32); 
        const texture = new THREE.TextureLoader().load(this.image); // afin de recuperer depuis les proprietés
        ...
        this.sphere = new THREE.Mesh( geometry, material );// passé en this pour pouvoir recuperer les propriétés
        this.scene.add( this.sphere );
    }

    // d) methode pour ajouter la sprite (tooltip)
    ...
    addTooltip (position, name) { 
        ...
        this.sprites.push(sprite) // permet d'ajouter au tableau sprite
    }
...
// e) sphere on creer une instance
