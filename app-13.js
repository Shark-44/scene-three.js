...
class Scene {
    ...
    createScene (scene) {
        ...
        this.points.forEach(this.addTooltip.bind(this))// ajout des points pour ajouter un tooltip .bind reference a l'objet
    }
    ...
    addTooltip (point) { //pas encore le nom
        ...
        sprite.name = point.name // recuperation du nom sur l'objet point
        sprite.position.copy(point.position.clone().normalize().multiplyScalar(30)); // via point
...
let s2 = new Scene('./test.jpg') // image scene 2

// avec la methode addPoint on position la tooltip et la scene a charger au click
s.addPoint({
    position:new THREE.Vector3( 45.51802272267461, -18.56677565020162, 8.924361641450966),
    name: 'intersecion',
    scene: s2 // qui est la scene a charger
})
// idem avec scene 2
s2.addPoint({
    position:new THREE.Vector3( 45.51802272267461, -18.56677565020162, 8.924361641450966),
    name: 'retour',
    scene: s // qui est la scene a charger
})
